...
(function(jQuery) {
	...
	jQuery.fn.cqlybestTag = function(options, tagToManipulate) {
		...
		var setupTypeahead = function() {
			...
			if (data) {
				...
				data.select = jQuery.proxy(tagManagerOptions.typeaheadOverrides.select, obj.data('typeahead'),
		...
		var refreshHiddenTagList = function() {
			...
			jQuery(lhiddenTagList).val(tlid.join(",")).change();
		};
		...
		var pushTag = function(tag, objToPush, isValid, id) {
			...
			if (jQuery.isFunction(tagManagerOptions.validator)) {
				if (!tagManagerOptions.validator(tag))
					return;
			}
			...
			if (alreadyInList) {
				var pTagId = tlid[p];
				jQuery("#" + objName + "_" + pTagId).stop().animate({
				...
				var execute = function() {
					...
					html += '<span class="myTag' + cl + '" id="' + newTagId + '"><span>' + tag
							+ '&nbsp;&nbsp;</span><a href="#" class="myTagRemover" id="' + newTagRemoveId + '" TagIdToRemove="'
							+ tagId + '" title="Remove">' + tagManagerOptions.tagCloseIcon + '</a></span> ';
					...
					jQuery("#" + newTagRemoveId).on("click", obj, function(e) {
						e.preventDefault();
						var TagIdToRemove = jQuery(this).attr("TagIdToRemove");
				...
				if (id) {
					...
					if (tagManagerOptions.AjaxPush != null) {
						if (/^[a-zA-Z0-9 ]{1,6}$/.test(tag)) {
							return;
						}
						// jQuery.post(tagManagerOptions.AjaxPush, { tag: tag });
						jQuery.post(tagManagerOptions.AjaxPush, {
							name : tag
						}, function(data) {
							if (data && data.id) {
								tagId = data.id + '';
		...
		return this.each(function() {
			...
			if (tagManagerOptions.hiddenTagListId == null) { /*
				...
				obj.after(html);
				obj.data("lhiddenTagList", obj.siblings("input[name='" + tagManagerOptions.hiddenTagListName + "']")[0]);
			} else {
			...
			obj.on("keypress", function(e) {
				...
				if (-1 != p) {
					...
					tagIsValid = true;
					var user_input = jQuery(this).val(); // user_input =
					// jQuery().inArray(delimeters[p]);
			...
			obj.change(function(e) {
				...
				if (data) {
					isSelectedFromList = $(this).data('typeahead').$menu.find("*").filter(listItemSelector).hasClass(
							selectedItemClass);
				...
				// console.log('Handler for .change() called, value selected:' +
				// obj.val());
				var ao = jQuery(".typeahead:visible");
				if (ao[0] != undefined) {
					// console.log('change: typeaheadIsVisible is visible');
					// when the user click with the mouse on the typeahead li element we
					// get the change event fired twice, once when the input field loose
					// focus and later with the input field value is replaced with li
					// value
					...
					if (isSelectedFromList) {
						...
						var item = $(this).data('typeahead').$menu.find(listItemSelector);
						var user_input = item.attr('data-value');
						user_input = trimTag(user_input);
						if (queuedTag == jQuery(this).val() && queuedTag == user_input) {
							isClear = true;
						} else {
							pushTag(user_input, null, true, item.attr('data-id'));
							queuedTag = user_input;
							// console.log('Handler for .change() called, typeahead value
							// pushed:' + queuedTag);
						}
